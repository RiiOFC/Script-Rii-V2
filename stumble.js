...
const asciiText = figlet.textSync("Safe", {
    font: 'Graffiti',
    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color(Id, "blueBright"), color(Username), color(Country, "green"), color(Region, "cyanBright"), color(Crowns, "blueBright"), color(SkillRating, "yellowBright")].join(" | ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
